...
def Calculate_Rewards(world, Route_length, segment_length=0,n=30):
   ...
   # print(cos_alpha)
    ...
    #print('brake',brake)
    Imediate_Reward= round(reward_velocity*(velocity/maximum_velocity)*(cos_alpha-abs(sin_alpha))*(velocity <= maximum_velocity) -reward_acc*(acceleration> max_acceleration)- stop_penalty*(velocity<=1) -steer_penalty*steering**2 + collision_reward + goal_reward*(1-goal_dis/Route_length) + At_goal_reward*(goal_dis<close_enough) -cross_error_penalty*(cross_track_error**2/40)*(abs(cross_track_error)>2) - round(brake_penalty*abs(brake),2),2)# Use this for penalizing lateral velocity
...
# round(reward_velocity*(velocity/maximum_velocity)*(cos_alpha-abs(sin_alpha))*(velocity <= maximum_velocity) -reward_acc*(acceleration> max_acceleration)- stop_penalty*(velocity<=1) -steer_penalty*steering**2 + collision_reward + goal_reward*(1-goal_dis/Route_length) + At_goal_reward*(goal_dis<close_enough) -cross_error_penalty*(cross_track_error**2/40)*(abs(cross_track_error)>2) - round(brake_penalty*abs(brake),2),2)
