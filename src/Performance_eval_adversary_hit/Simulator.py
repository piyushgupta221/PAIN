...
class World(object):
    def __init__(self, carla_world, hud, actor_filter, route, random_initialization):
        ...
        self._weather_index = 0  # default 0
        self._actor_filter = actor_filter
        self.restart(random_initialization)
        self.world.on_tick(hud.on_world_tick)
        self.next_weather()

    def restart(self, random_initialization):
        ...
        while self.player is None:
            ...
            rand_id=np.random.randint(0,len(self.Route))
            if random_initialization:
                init_transform =self.Route[rand_id]
            else:
                init_transform =self.Route[0]
...
def choose_pid_waypoint2(index,route,player,world):
    if index>len(route)-1:
            index=len(route)-1
    pid_waypoint = route[index]
    player_loc = player.get_location()
    #print(np.linalg.norm([pid_waypoint.location.x -route[index+10].location.x, pid_waypoint.location.y-route[index+10].location.y]))
    dist = np.linalg.norm([pid_waypoint.location.x - player_loc.x , pid_waypoint.location.y-player_loc.y])
    while dist < 10:
        index+=2
        dist = np.linalg.norm([pid_waypoint.location.x - player_loc.x , pid_waypoint.location.y-player_loc.y])
        if index>len(route)-1:
...
def choose_pid_waypoint(index,route,player,world):
    #if index>len(route)-1:
    #        index=len(route)-1
    #pid_waypoint = route[index]
    #player_loc = player.get_location()
    #world=player.get_world()
    _,ix =Reward.closestRoutePoint(world,start_index=0)
    index=ix+5
    if index>len(route)-1:
       index=len(route)-1
          #  break
...
def pretrain_memory(args):
   ...
   while mem.tree.data[ Q_network.pretrain_length-1] ==0:
       ...
       print("Avoider: Number of experiences in pretrain memory = ", Q_network.memory_size- np.sum(mem.tree.data==0))
...
def execute_DQN(args,DQNetwork, TargetNetwork, file_name=""):
   with tf.Session() as sess:
      ...
      # Load the model
      saver.restore(sess,file_name +".ckpt")
      path=file_name+"perf_results.txt"
      game_loop(args, memory=None, explore_exploit_action=False, decay_step=0, episode=0,  tau=0, DQNetwork=DQNetwork, TargetNetwork=TargetNetwork, writer=None, write_op=None, training=False, sess=sess, use_model=True, path=path)
   return
...
# ==============================================================================
# -- Shift ROute to center of road---------------------------------------------------------------
# ==============================================================================

def road_center(ROUTE):
       center_route=[]
       #Road_center=carla.Waypoint
       #f = open("transforms.txt",'w')
       for g in range(len(ROUTE)):
            current_waypoint=ROUTE[g][0]
            right_waypoint=current_waypoint
            left_waypoint=current_waypoint
            right_index=right_waypoint.lane_id
            left_index=left_waypoint.lane_id
            while right_waypoint.get_right_lane() !=None and right_waypoint.get_right_lane().lane_type == carla.LaneType.Driving:
                right_waypoint=right_waypoint.get_right_lane()   
                #print("Lane_id=",right_waypoint.lane_id, "road_id=",right_waypoint.road_id,"section_id=",right_waypoint.section_id,"lane_change", right_waypoint.lane_change, "lane_type",right_waypoint.lane_type) 
                right_index=right_waypoint.lane_id
            #print("right_id=",right_index)
            while left_waypoint.get_left_lane() !=None and left_waypoint.get_left_lane().lane_type == carla.LaneType.Driving:
                left_waypoint=left_waypoint.get_left_lane()   
                #print("Lane_id=",left_waypoint.lane_id, "road_id=",left_waypoint.road_id,"section_id=",left_waypoint.section_id,"lane_change", left_waypoint.lane_change, "lane_type",left_waypoint.lane_type) 
                left_index=left_waypoint.lane_id
            #print("right_id=",right_index,"left_id=",left_index)

            
            
            mid_lane_id_low=math.floor((right_index+left_index)/2.0)
            mid_lane_id_high=math.ceil((right_index+left_index)/2.0)
            waypoint_left=False
            waypoint_right=False
            right_waypoint=current_waypoint
            left_waypoint=current_waypoint
            l_waypoint=current_waypoint
            r_waypoint=current_waypoint
            #print("lane_type", current_waypoint.lane_type)
            #print(int(mid_lane_id_low), int(mid_lane_id_high))
            while right_waypoint.get_right_lane() !=None and right_waypoint.get_right_lane().lane_type == carla.LaneType.Driving:
                 right_waypoint=right_waypoint.get_right_lane()
                 if right_waypoint.lane_id==int(mid_lane_id_low):
                    l_waypoint=right_waypoint
                    waypoint_left=True
                    #print(right_waypoint.lane_id, int(mid_lane_id_low))
                 if right_waypoint.lane_id==int(mid_lane_id_high):
                    r_waypoint=right_waypoint
                    waypoint_right=True
                    #print(right_waypoint.lane_id, int(mid_lane_id_high))
                 if waypoint_left and waypoint_right:
                    break
                 #print("lane_type", left_waypoint.lane_type)
            while left_waypoint.get_left_lane() !=None and left_waypoint.get_left_lane().lane_type == carla.LaneType.Driving:
                 left_waypoint=left_waypoint.get_left_lane()
                 #print(left_waypoint.lane_id, int(mid_lane_id_low))
                 if left_waypoint.lane_id==int(mid_lane_id_low):
                    l_waypoint=left_waypoint
                    waypoint_left=True
                    #print(left_waypoint.lane_id, int(mid_lane_id_low))
                 if left_waypoint.lane_id==int(mid_lane_id_high):
                    r_waypoint=left_waypoint
                    waypoint_right=True
                    #print(left_waypoint.lane_id, int(mid_lane_id_high))
                 if waypoint_left and waypoint_right:
                    break
                 #print("lane_type", left_waypoint.lane_type)
            ...
            pitch=(l_waypoint.transform.rotation.pitch +r_waypoint.transform.rotation.pitch)/2.0
            ...
            '''
            f.write('%f, %f, %f, %f\n' %   (Road_center_transform.location.x,Road_center_transform.location.y,Road_center_transform.location.z,Road_center_transform.rotation.yaw))
            f.write('%f, %f, %f, %f\n' % (r_waypoint.transform.location.x,r_waypoint.transform.location.y,r_waypoint.transform.location.z,r_waypoint.transform.rotation.yaw))
            f.write('%f, %f, %f, %f\n' %  (l_waypoint.transform.location.x,l_waypoint.transform.location.y,l_waypoint.transform.location.z,l_waypoint.transform.rotation.yaw))
            '''
...
def get_transform(vehicle_location, angle, d=6.4):
    ...
    location = carla.Location(d * math.cos(a), d * math.sin(a), 20.0) + vehicle_location
    return carla.Transform(location, carla.Rotation(yaw=angle, pitch=-35))
...
def game_loop(args, data_log=None, memory=None, explore_exploit_action = False, decay_step=0, episode=0, tau=0, DQNetwork=None, TargetNetwork=None, writer=None, write_op=None, training=False, sess=None, use_model=False, path=""):
    
    f = open("avoider_game_status",'w')
    f.write("0")
    f.close()
    x = 0
    y = 0
    ...
    try:
       ...
       # world = World(client.get_world(), hud, args.filter)
        ...
        #follow car initial position
        spectator = WORLD.get_spectator()
        all_actors= WORLD.get_actors()
        all_vehicles = all_actors.filter('vehicle*') 
        protagonist = all_vehicles.filter('vehicle.tesla*')[0]
        ...
        #print(ROUTE)
        Route_length =Reward.initial_dis_to_goal(world)
        num_points_on_seg=30  # not using
        segment_length = Reward.segment_len(world,n=num_points_on_seg)  # not using
        ...
        pure_pid = 0   # np.random.rand()
        kill=0
        spectator.set_transform(get_transform(protagonist.get_location(), protagonist.get_transform().rotation.pitch + 180, d=-50))
        collision_bp = world.world.get_blueprint_library().find('sensor.other.collision')
        colision_transform = carla.Transform(carla.Location(x=-50, z=20))
        collision = world.world.spawn_actor(collision_bp, colision_transform, attach_to=protagonist)
        spectator.set_transform(collision.get_transform()) 
        spectator.set_transform(get_transform(protagonist.get_location(), protagonist.get_transform().rotation.pitch + 180, d=-50))
        while not done and not kill:
            clock.tick_busy_loop(60)
            pid_index=choose_pid_waypoint(pid_index,ROUTE,world.player,world)
            #print(pid_index)
            v = world.player.get_velocity()
            current_state=next_state
            current_state_vehicle=next_state_vehicle
            if explore_exploit_action:# and steps%2==0:
                target_waypoint_transform = ROUTE[pid_index]
                ...
                pid_speed=Reward.max_velocity*3.6/1.1 #pid_speed_previous+3.6# math.sqrt(v.x**2 + v.y**2 + v.z**2)+2
                ...
                #print(text[choice])
            #elif explore_exploit_action and steps%2 !=0:
            #    pass
            #   chosen_action=chosen_action   #frame_skip
                ...
                #chosen_action = possible_actions[6]
                #print(chosen_action)
            else:
            ...
            if steps%25==0: # and decay_step%5==0:       
                 spectator.set_transform(collision.get_transform()) 
                 #spectator.set_transform(get_transform(protagonist.get_location(), protagonist.get_transform().rotation.roll -190 , d=-40))
            ...
            if len(stuck)==20:
                if np.sum(velocity)/len(velocity)<0.1:   # np.sum(throttle_commands)/len(throttle_commands)>0.6 and np.sum(throttle_commands)/len(throttle_commands)>0.6 and
                     vehicle_stuck=True
                     total_reward=-10000000
                     print('Avoider: Vehicle_stuck',vehicle_stuck)
            goal_dis=Reward.dis_to_goal(world)
            #print(goal_dis)
            done= len(world.collision_sensor.history)>0 or steps>Q_network.max_steps or vehicle_stuck or goal_dis<Reward.close_enough
     
            _,p_index=Reward.closestRoutePoint(world,start_index=0)
            if p_index>100:
               kill=1
            if done:
        ...
        f.write("1")
        f.close()   
        if training: # and decay_step%5==0:
            ...
            for y in range(0, 20):
                ...
                #print(Qs_target_next_state[2:4,:])
                ...
                writer.flush()
                print('Avoider: Episode: {}'.format(episode),
                          'Total reward: {}'.format(total_reward),
                          'Training loss: {:.4f}'.format(loss), 'Simulation_time: {:.2f}'.format(hud.simulation_time-last_episode_end_time),
                          'Explore P: {:.4f}'.format(explore_probability))
                data_log.write('%f, %f, %f, %f' % (total_reward, loss, explore_probability, hud.simulation_time-last_episode_end_time))         
                data_log.write('\n')
                if tau > Q_network.max_tau:
                    ...
                    sess.run(update_target)
                    tau = 0
                    print("Avoider: Model updated")


        completion_time=hud.simulation_time
        if use_model:
              Collsion_intensity=0
              if len(world.collision_sensor.history)>0:
                 Collsion_intensity= world.collision_sensor.history[0][1]
              ...
              if len(collide)>0:
                  for c in collide:
                      if "vehicle" in c.type_id:
                          Hit=1
              #dis_covered= Route_length-goal_dis
              #MTTF=completion_time-last_episode_end_time
              performance_log = open(path,'a+') # Reward, Loss Exploration, probability, mean time to failure
              performance_log.write('%f, %f' % (Hit, Collsion_intensity))         
              performance_log.write('\n')
              performance_log.close()
              print('Avoider: Congratulations !!! Successfully used trained model')
              print('Avoider: Total reward: {}'.format(total_reward))
...
def main():
    ...
    try:
        ...
        file_name = "./models/model"
        # If training agent
        if Q_network.Train_network:
            if Q_network.TransferLearning:
                memory=Memory(capacity=Q_network.memory_size)
                try: 
                    memory.load(file_name)
                    print("Avoider: memory decay_step after loading",memory.decay_step, memory.explore_prob)
                except:
                    print('Avoider: No Pretrain Memory Exist. Initializing Memory!!!')
                    memory =pretrain_memory(args)
                    memory.save(file_name)
            else:
                print('Avoider: Initializing Memory!!!')
                memory =pretrain_memory(args)
                memory.save(file_name)
            print('Avoider: Memory Experiences Initialized. Starting to train the network!!!')
            ...
            file_name=trainQ_network(args, memory,data_log, DQNetwork, TargetNetwork,  writer, write_op, training=Q_network.Train_network, TransferLearning=Q_network.TransferLearning,file_name=file_name)
            print('Avoider: Network training complete!!! Lets play')
            data_log.close()
        '''
        if auto:   
           os.system('sudo python spawn_npc.py -n 20 -w 0 --safe -autopilot True &')
        else:
           os.system('sudo python spawn_npc.py -n 20 -w 0 --safe &')
        '''
