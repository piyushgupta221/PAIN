    ...
    try:
       ...
       # world = World(client.get_world(), hud, args.filter)
        ...
        while not done:
            ...
            if len(stuck)==20:
                if np.sum(velocity)/len(velocity)<0.1:   #np.sum(throttle_commands)/len(throttle_commands)>0.6 and np.sum(velocity)/len(velocity)<0.2:
                     vehicle_stuck=True
        ...
        if use_model:
              ...
              performance_log.write('%f, %f, %f, %f' % (total_reward, MTTF, dis_covered, Collsion_intensity))         
